...
public class ResourcesSelectionWindow extends JFrame {
    ...
    private AssignRequestWindow parentFrame;

    private List<Pair<Resource, Integer>> resourcesSelected = new ArrayList<>();
    ...
    public ResourcesSelectionWindow(AssignmentsDepartment assignmentsDepartment, AssignRequestWindow aParentFrame) {
    ...
    private JComboBox<MobileResource> makeResourcesComboBox() {
        EasyComboBoxModel<MobileResource> comboModel = new EasyComboBoxModel<>(getResources());
        JComboBox<MobileResource> combo = new JComboBox<>(comboModel);
        ...
        combo.addActionListener(anEvent -> {
            // TODO refactor me!
            if (!(((JComboBox<Resource>) anEvent.getSource()).getSelectedItem() == null)) {
    ...
    private List<MobileResource> getResources() {
        List<MobileResource> resources = new ArrayList<>();
        department.getResourcesDepartment().getMobileResources().stream()
                .filter(mobileResource -> !resources.contains(mobileResource))
    ...
    private void createAddResourceButtonListeners() {
        addResourceButton.addActionListener(anEvent -> {
            resourcesSelected.add(new Pair<>(resource, quantity));
            resource = null;
            quantity = null;
    ...
    private void createResourcesTable() {
        ReadOnlyTableModel<Pair<Resource, Integer>> tableModel = new ReadOnlyTableModel<>(resourcesSelected);
        ...
        resourcesTable.getSelectionModel().addListSelectionListener(
                ResourcesSelectionWindow.this::whenResourcesTableSelectionChanged);
